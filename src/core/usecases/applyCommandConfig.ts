 ...
 * take hold by RESPAWNING the agent (model and effort are both
 * spawn-time over ACP) is skipped once a conversation exists — the
 * respawn makes the next turn re-ingest the whole conversation, and
 * saving those tokens outranks honouring the command's preference. The
 * command then simply runs under the session's current setup, and the
 * toolbar (truthfully) doesn't move.
 *
...
export class ApplyCommandConfig {
  ...
  async execute(tabId: string, prompt: string): Promise<void> {
    ...
    // Only fields that actually differ are applied: model and effort
    // respawn the tab's agent session (both are spawn-time over ACP), so
    // re-running a command with the setup already in place must cost
    // nothing.
    ...
    // A conversation in flight makes either respawn too expensive (see
    // the class comment); before the first turn both are free. The guard
    // also keeps a command from queueing a change the user never asked
    // for — mid-conversation these use cases DEFER rather than apply.
    if (tab.project.providerSessions[tab.project.provider]) return;
    if (config.model !== undefined && config.model !== (tab.project.model ?? "")) {
      await this.selectModel.execute(tabId, config.model);
    }
    if (config.effort !== undefined && config.effort !== (tab.project.effort ?? "")) {
